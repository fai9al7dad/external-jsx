...
function parseDirectory(directoryPath: string): void {
  ...
  files.forEach((file) => {
    ...
    if (stats.isDirectory()) {
      ...
      if (file.endsWith(".tsx") && !file.endsWith(templateFileExtension)) {
        ...
        if (templateTag){
          let dirValue = templateTag[0].split(
            /"|'.+"|'/,
        )[1]
          if (!dirValue.endsWith(templateFileExtension)){
            new Error(`Template file ${dirValue} does not have the correct extension. Expected ${templateFileExtension}`)
          }
